                  ...
                  {bookings.map(b => (
                    <div className="booking-item card" key={b.id}>
                      ...
                      <div className="bi-details">
                        ...
                        <div className="bi-detail bi-total"><span>💰 Total</span><strong>LKR {fmt(b.total_price)}</strong></div>
                      </div>
                  ...
                  {orders.map(o => (
                    <div className="order-item card" key={o.id}>
                      ...
                      <div className="order-items-list">
                        {(o.items||[]).map((item,i) => (
                          <span key={i}>{item.name} ×{item.qty} — LKR {(item.price*item.qty).toFixed(2)}</span>
                        ))}
                      </div>
